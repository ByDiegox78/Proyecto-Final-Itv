...
public class CitasXmlStorage : ICitasXmlStorage {
    ...
    public CitasXmlStorage() {
        InitStorage();
    }
    private readonly ILogger _logger = Log.ForContext<CitasXmlStorage>();
    ...
    public Result<bool, DomainError> Salvar(IEnumerable<Cita> items, string path) {
        try {
            ...
            var serializer = new XmlSerializer(typeof(List<CitaDto>)); // Convertir la lista a Xml 
    ...
    public Result<IEnumerable<Cita>, DomainError> Cargar(string path) {
        ...
        try {
            ...
            var dtos = serializer.Deserialize(streamReader) as List<CitaDto>;
            if (dtos == null) 
                return Result.Failure<IEnumerable<Cita>, DomainError>(StorageErrors.InvalidFormat("El archivo XML no tiene un formato válido o está vacío"));
